...
# Question:
rep_table.m$age <- (rep_table.m$distance / (2* 8.22e-9))# Calculate using the substitution rate and the formula provided in the tutorial
hist(rep_table.m$age)
...
#own experiment, try with age range as well:
ggplot(rep_table.m, aes(fill = fam, x = age, weight = value / 1000000)) +
  ...
  xlab("Age") +
  ylab("Sequence (Mbp)") +
  theme(axis.text.x = element_text(angle = 0, vjust = 1, size = 9, hjust = 1), plot.title = element_text(hjust = 0.5))

ggsave(filename = "Plots/output_age.pdf", width = 10, height = 5, useDingbats = F)
